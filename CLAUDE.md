...
```bash
pnpm --filter @app/infra run deploy --all --require-approval never    # 両スタックを一括デプロイ（deploy は pnpm 組み込みコマンドと衝突するため run が必須）
```
...
- **`TAVILY_API_KEY` を必ずデプロイ前に設定する** — `AgentcoreSlackAgent` を含むデプロイ（例: `pnpm --filter @app/infra run deploy --all --require-approval never`）の前に環境変数として設定すること。未設定だと Web 検索が実行時に静かに失敗する（例外ではなく LLM にエラー文字列として返る）。SlackBot だけの更新は `--exclusively` で AgentCore Runtime を触らない。
